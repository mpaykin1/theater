...
function render(){
if(step>=qs.length){const individual=answers[3]?.includes('одну конкретную');const route=individual?'индивидуальная работа над конкретной жизненной сценой':'курс «Проявись» для системной тренировки самовыражения';title.textContent='Похоже, вам стоит начать отсюда';box.innerHTML='';result.hidden=false;result.innerHTML=`<div class="quiz-result"><h4>${route}</h4><p>Это не диагноз и не ярлык. Это рабочая гипотеза: какой формат сейчас даст вам больше практики и ясности.</p><a class="btn" target="_blank" rel="noopener" href="${wa('Здравствуйте! Я прошёл(ла) «Сейчас мы выясним». Мои ответы: '+answers.map((a,i)=>`${i+1}) ${a}`).join('; ')+' . Хочу понять, какой формат мне подойдёт.')} ">Отправить мои ответы Михаилу →</a></div>`;label.textContent='Маршрут готов';return}
